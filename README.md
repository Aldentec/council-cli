...
- **Team memory** - when persistence is on, advisors evolve across meetings from first-timers to long-term experts
- **Ask about history** - questions like "What did we talk about last time?" summon the full team to reference past summaries
...
settings:
  ...
  persist_sessions: true              # enables long-term team memory + meeting count tracking
  max_sessions_loaded: 3              # number of recent team meetings injected into context
```

When `persist_sessions` is enabled, Council stores each meeting and tracks frequency per team composition. Advisors treat meeting #1 as a first introduction (limited product familiarity) and become more context-aware experts in later meetings by building on prior summaries. If session persistence is off, this long-term memory does not exist.
...
1. **All-room cues** - if your message contains "everyone", "all of you", "the whole room", **or history questions like "last time", "what did we talk about"**, every advisor responds.
...
## Team memory and persona evolution

When session persistence is enabled (`persist_sessions: true`), Council tracks how many times the **same team** meets on this project. On the first meeting, advisors know only the project briefing — they do not pretend product familiarity they haven't earned. By meeting #3, #5, and beyond, summaries of earlier meetings are injected into context, allowing the team to build on prior decisions and act as seasoned insiders rather than role-only actors.

Each session is tagged with a `team_id` derived from the project name plus every agent's (name, role, persona) triple. Only sessions sharing the same `team_id` count toward that team's `team_meeting_number`. If you swap one advisor, you effectively become a new team and the count resets.

Past meeting summaries are prepended to the shared context before the current session starts (up to `max_sessions_loaded` most recent ones). Advisors will acknowledge and follow up on earlier decisions, open questions, and dissenting views across meetings.

**Ask about previous meetings** — when memory is enabled, questions like *"What did we talk about last time?"*, *"What were our action items?"*, or *"What did we decide?"* trigger all advisors to summarize the relevant historical takeaways from past sessions.
